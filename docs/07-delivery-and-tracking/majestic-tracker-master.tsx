...
const tasks = [
...

  {
    ...
    items: [
      ...
      { id: 65, task: "Avatar motion rules",                                  status: "complete", note: "Gesture and posture library, portal system, two portal shapes, arrival/exit animations — Avatar Motion Rules & Presence Spec v1.0", done: true, today: false, mode: "chat", offline: false, dependsOn: [] },
  ...
  },
  {
    ...
    items: [
      ...
      { id: 169, task: "Navigation architecture spec — three-state spatial system",  status: "complete", note: "LOCKED June 2026. majestic-navigation-architecture-spec.md. Ground/Outer Realm/Self State. Nameless to users. Bottom nav: Home/Codex/Reading/Journal. Portal up to Codex, door down to Self. Reading/Journal lateral fade same state. Skippable after first use.", done: true, today: false, mode: "chat", offline: false, dependsOn: [] },
      { id: 170, task: "Ground state / Command center — visual spec",               status: "complete", note: "LOCKED June 2026. majestic-home-screen-spec.md + majestic-navigation-architecture-spec.md. Threshold City POV through panoramic window. HUD corners + sector coords + AURA LINK pulse. Console hardware frame. City skyline with avatar-accent neon, rain, moon, atmospheric haze.", done: true, today: false, mode: "chat", offline: false, dependsOn: [] },
      { id: 171, task: "Home screen content system spec — Co-Star + pop culture refs", status: "complete", note: "LOCKED June 2026. majestic-home-screen-spec.md. Co-Star register: second person, fragment, no hedging. Daily directive (card-informed, world voice). Pop culture reference card (film/song/history/quote/artwork, avatar-filtered). Observational prompts: Watch for / Be mindful of / Notice today — NOT a to-do list. Freshness: 5-6 refs per card per avatar, 30-day no-repeat rule.", done: true, today: false, mode: "chat", offline: false, dependsOn: [] },
      { id: 172, task: "Portal visual treatment — define reference, mood, mechanic",  status: "todo",  note: "BLOCKER for #109 and #177. What does the wormhole/portal actually look like? Luke must provide reference image, mood direction, or visual brief before this can go to design or build. Currently the most important unresolved design decision in the product.", done: false, today: false, mode: "offline", offline: true, dependsOn: [] },
      { id: 173, task: "Self state desk scene — Lottie animation spec",              status: "todo",  note: "Two animation events: (1) Book opening — journal entry, 600-800ms. (2) Drink pouring — altar/reading initiation, ritualistic. Also: lateral pan mechanic between altar (right) and journal (left) within self state. Format: Lottie JSON, NOT GIF. Write spec before commissioning animation.", done: false, today: false, mode: "chat", offline: false, dependsOn: [154] },
      { id: 174, task: "Self state environments — 4 avatar-specific desk scenes",    status: "todo",  note: "Asset production: Destiny/seascape, Olivia/field, Eli/urban cityscape, Casper/high-rise interior. Each needs ambient background (video loop or parallax still) + Lottie desk objects. Also: idle/default desk state before Reading or Journal selected. Four scenes total — significant asset commitment.", done: false, today: false, mode: "offline", offline: true, dependsOn: [173] },
      { id: 175, task: "Pop culture reference database — Major Arcana × 4 avatars",  status: "todo",  note: "Research phase first (Oso). Then content production: 22 Major Arcana × 4 avatar filters × 5-6 reference types each = ~500-550 reference units for Major Arcana. Types: film, song, historical moment, artwork, quote. Each needs: reference + 1-2 sentence through-line back to card + 2-3 observational prompts. Separate chat/project.", done: false, today: false, mode: "chat", offline: false, dependsOn: [171] },
      { id: 176, task: "Avatar colour temperature per nav state — implement",          status: "todo",  note: "City neon glow at ground state shifts subtly per active avatar: Destiny/teal-dominant, Casper/rust-warm, Eli/blue-grey, Olivia/green. One CSS variable per avatar. Pending Luke confirmation — flag in decision brief first.", done: false, today: false, mode: "code", offline: false, dependsOn: [102, 172] },
      { id: 177, task: "Portal transition animation — production",                    status: "todo",  note: "Lottie or video animation for portal/wormhole (Outer Realm) and door-opening (Self State) transitions. Cannot begin until #172 portal visual treatment is defined by Luke. Potential external commission.", done: false, today: false, mode: "offline", offline: true, dependsOn: [172] },
    ]
  },
...

  {
    ...
    items: [
      ...
      { id: 146, task: "Error monitoring — Sentry integration",              status: "complete", note: "Install @sentry/react-native. Catches JS errors, native crashes, performance issues. Do before beta so you have visibility from day one. Without this you're flying blind.", done: true, today: false, mode: "code", offline: false, dependsOn: [132] },
      { id: 147, task: "Analytics — PostHog integration",                    status: "complete", note: "Install posthog-react-native. Track onboarding completion, daily draw rate, D3/D7/D30 retention, avatar distribution. Do before beta — you need data from the first user.", done: true, today: false, mode: "code", offline: false, dependsOn: [132] },
      { id: 178, task: "Analytics — migrate GA4 track* calls to PostHog",       status: "todo", note: "src/lib/analytics/index.ts gtag() no-ops on iOS/Android, so every existing trackNavigationClick/trackQuizAnswer/etc. call is web-only. PostHog (#147) now runs on all platforms alongside GA4. Move the existing call sites over (or dual-fire) so the onboarding funnel has native data. Deferred from wave 1 as scope creep.", done: false, today: false, mode: "code", offline: false, dependsOn: [147] },
...
export default function MajesticTracker() {
  ...
  const flags = [
    { text: "BLOCKER: Portal visual treatment (#172) undefined — Luke must provide reference/mood before #109 or #177 can proceed", severity: "red" },
    { text: "DECISION NEEDED: Codex expansion eligibility trigger — subscription, streak, or both? Affects RevenueCat (#115)", severity: "amber" },
    { text: "DECISION NEEDED: Avatar colour temperature at ground state — city neon shifts per avatar? (#176)", severity: "amber" },
    { text: "DECISION NEEDED: Daily draw method — algorithmic, user-selected, or hybrid? Affects home screen UX", severity: "amber" },
    { text: "DECISION NEEDED: Drink pour animation — universal or avatar-specific? Affects #173 scope", severity: "amber" },
    ...
    { text: "New specs created this session — upload to project knowledge: majestic-navigation-architecture-spec.md, majestic-home-screen-spec.md, updated codex-spec.md, updated journal-spec.md", severity: "slate" },
