...
{ id: 154, task: “Altar & ritual spec — avatar altars, talismans and breath mechanic”, status: “complete”, note: “Locked v2.0. majestic-altar-ritual-spec-v2.md. Four-layer visual hierarchy: Light Source / Talisman / Reading Surface / Element. Talisman replaces generic hold gesture as ritual initiator. 5s breath max. Onboarding compressed exhale-only. Reflection Mode flagged for v2. ADDENDUM: Fan selection state now inserted between talisman hold and reveal for initiated readings — spec in majestic-fan-selection-spec.md. Daily draw unchanged.”, done: true, today: false, mode: “chat”, offline: false, dependsOn: [121, 122] },
...
{ id: 165, task: “Jumping card — server-side scoring algorithm”,        status: “todo”, note: “Composite score from: streak_count, session_depth_seconds, reading_frequency, birth_card_recurrence, last_jump_date, milestone flags. Score crosses threshold → next initiated reading triggers jump. Cooldown: max 1 per 7 days. Supabase fields needed: last_jump_date (timestamp), jump_score (int), session_depth_seconds (int — add to existing session tracking). Card selection logic: birth card recurrence → personality/soul card jumps; milestone → thematic card; otherwise → recent recurrence card. Spec: majestic-fan-selection-spec.md Part 04.”, done: false, today: false, mode: “code”, offline: false, dependsOn: [111, 107] },
{ id: 166, task: “Jumping card — avatar copy (8 lines)”,                status: “todo”, note: “8 lines locked in majestic-fan-selection-spec.md Part 04.5 and 04.6. 4 pre-reveal lines (one per avatar, fires as card holds elevated before flip) + 4 post-reveal lines (appended after standard interpretation). Add to avatar copy library and wire into reading screen companion line logic. Pre-reveal fires regardless of card identity. Post-reveal fires 800ms after standard companion line settles.”, done: false, today: false, mode: “chat”, offline: false, dependsOn: [107] },
{ id: 167, task: “Fan selection — asset repo placement”,                status: “todo”, note: “No new image assets required for fan. Card backs use existing master Majestic emblem at 40% opacity + avatar accent wash 15%. Haptics via expo-haptics — no assets. Journal jump marker uses existing marker-signal-node.svg from app/assets/glyphs/markers/. App icon (when finalised): app/assets/icon.png. Card back design (when finalised from MJ iteration): app/assets/card-back.png at 1024×683px minimum. Fan spec: majestic-fan-selection-spec.md.”, done: false, today: false, mode: “offline”, offline: true, dependsOn: [107] },
...
{ id: 107, task: “Card flip and reveal animation”,                     status: “todo”, note: “Full sequence: talisman hold → 13-card bouquet fan rises → user selects card → fan folds → reveal. Jumping card fires mid-rise when triggered (non-ignorable). Spec locked: majestic-fan-selection-spec.md. Reveal sequence unchanged per majestic-reading-screen-animation-aura-spec.md 02.4. Haptics: expo-haptics, 4 trigger points. Daily draw unaffected — auto-reveal only. Depends on #165 (scoring) and #166 (copy) before jumping card logic can be built.”, done: false, today: false, mode: “code”, offline: false, dependsOn: [85, 77, 165, 166] },
