...
**New specs added post-v3.1 (May 2026):** - Subscription and monetisation model fully defined →  `majestic-subscription-tier-spec.md` - Authentication timing and positioning defined →  `majestic-auth-monetisation-spec.md`- Altar, talisman, and ritual system defined →  `majestic-altar-ritual-spec-v2.md` - Pixel Elder lamp sequence and reading initiation role defined → `majestic-pixel-elder-addendum.md` - Dig Deeper access model updated for subscription gating →  `majestic-dig-deeper-spec-v2.md` - Design system (spacing, components, icons) defined →  `majestic-design-system.md` - Quarter deck fan selection and jumping card phenomenon defined → `majestic-fan-selection-spec.md`

**Card selection model (initiated readings):** The talisman hold now initiates a 13-card bouquet fan — cards rise from the talisman base point, fanning outward at 7° intervals with a 12° forward perspective tilt toward the user. The user taps to select. Haptic feedback at four points: hold threshold, fan land, card tap, reveal apex. Daily draw retains auto-reveal (no fan). The jumping card phenomenon fires on initiated readings only when a composite behaviour score is met — streak, session depth, birth card recurrence, milestone sessions. When triggered, one card surges mid-rise before the user can interact; the reading begins without selection. Non-ignorable. Maximum one per 7-day period. Jump marker persists in journal using `marker-signal-node.svg`. Full spec: `majestic-fan-selection-spec.md`.

**Asset repo placement for fan and jumping card:** No new image assets required for the fan itself. Card backs use the existing master Majestic emblem. Jump marker in journal uses `app/assets/glyphs/markers/marker-signal-node.svg` (already exported). Card back final design (from MJ iteration) → `app/assets/card-back.png` at 1024×683px minimum. App icon final → `app/assets/icon.png` at 1024×1024px.
