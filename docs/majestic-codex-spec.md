...
The Codex is accessed via the **Outer Realm state** in the three-state navigation system. When the user taps Codex in the bottom nav, a portal/wormhole transition fires upward and the environment shifts to a cosmic, galactic atmosphere. The Codex content lives within this state.

This does not change the Codex content spec — it changes the environmental wrapper and how the user arrives. See `majestic-navigation-architecture-spec.md` for full transition details.

The Codex tab remains the second item in the bottom navigation bar. Active accent: Eli slate `#A8B4C8`.
...
## 12 — CODEX EXPANSION — TWO-TIER SYSTEM (June 2026)

**Status: Locked concept. Eligibility trigger pending Luke decision.**

The Codex operates across two layers. From the user perspective it is always one Codex — it simply grows.

### Base Layer — All authenticated users

The full 78-card deck, card meanings, world lore (unlocked on first draw), avatar-voiced interpretation line, and draw history. This is the Codex as currently specced in sections 01–11.

### Expanded Layer — Unlocked when eligible

When a user becomes eligible, additional content surfaces within the same Codex structure — no new section, no separate tab. Content appears where it did not exist before. The Codex visibly grows.

Expanded content includes:

- **Avatar-specific card interpretations** — the same 78 cards filtered through the active avatar full interpretive voice (distinct from the one-liner in base layer)
- **Avatar lore and backstories** — Threshold City world-building, character history, deeper narrative of each companion
- **Gamified unlock content** — anything earned through the gamification layer is permanently archived and accessible here

**Visual treatment of locked expanded content:** Present at 30% opacity with a subtle locked indicator. Not a padlock. Like a card that has not yet woken up. Matches the undiscovered card treatment philosophy (see Section 04).

**Eligibility trigger — TBD, Luke decision required.**
Options: subscription, streak milestone, avatar bond level, or combination. This directly affects RevenueCat integration (#115) and gamification logic. Must be resolved before build.

**Pitch rationale:** The Codex becomes the reason users subscribe AND the reason they stay. It is a living record of how far they have come into the world. Rewards investment with depth, not just features.

### Supabase additions required

- user_codex_unlocks.lore_unlocked (bool) — avatar lore eligibility
- user_codex_unlocks.expanded_interp_unlocked (bool) — avatar interpretation eligibility
- Eligibility check function server-side — evaluates trigger and updates flags
- Expanded content storage (JSONB or dedicated table) for avatar lore per user
