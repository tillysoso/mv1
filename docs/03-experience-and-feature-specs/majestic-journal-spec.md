...
## 00 — NAVIGATION CONTEXT UPDATE (June 2026)

The Journal is accessed via the **Self State** in the three-state navigation system. When the user taps Journal from the bottom nav, a door-opening transition fires downward and the environment shifts to the desk/interior atmosphere. The journal content lives within this state.

Within the Self State, the Journal and Reading destinations share the same environmental layer. The transition between them is a lateral desk pan — no portal required. The desk shifts slightly left to reveal the open book (Journal), or slightly right to reveal the altar (Reading).

See majestic-navigation-architecture-spec.md for full transition details.
...
## 02A — SELF STATE ENVIRONMENT — DESK SCENE (June 2026)

The Journal sits within the Self State, which uses a persistent animated desk stage at the bottom of the screen. This is the environmental anchor for both Reading and Journal.

**The desk stage:**

- Bottom panel of the screen, persistent across the Self State
- Animated using Lottie (NOT GIF — performance requirement)
- Viewed from a slight overhead angle
- Contains objects appropriate to a personal ritual/writing space

**Journal orientation — slightly left:**
When the user is in the Journal, the desk stage pans slightly left. A book opens — animated, real book-opening feel. Once open, the journal UI sits above this environmental layer as the primary content.

**Book opening animation (Lottie):**

- Triggered on entry to Journal from any state
- Book closed → spine visible → covers part → pages fall open → settle
- Duration: approximately 600–800ms
- After settle: desk loops in idle ambient state (candle flicker, subtle movement)

**Avatar-specific environments:**
The self state background (behind the desk) is avatar-specific:

- Destiny → seascape visible through a window or atmosphere
- Olivia → a field or natural landscape
- Eli → an urban cityscape
- Casper → a high-rise interior

**Idle/default state:** TBD — the desk resting position before Journal or Reading is selected. To be defined.

**Asset format:** Lottie JSON for animation events. Video (mp4) or parallax still for ambient background. Never GIF.
...
1. A blank entry panel rises from the bottom — same modal behaviour as Codex detail view
1. **Intention field** appears first — one line, notebook margin style. Placeholder: *“What’s moving through you right now?”* Low contrast, no border, disappears on first tap. Optional — skip by tapping the writing area below.
1. **Note field** — the main writing area. Full width. Montserrat. Bone white on `#16213E`. Minimum 5 lines visible, expands as the user types. Keyboard-aware — content sits above keyboard naturally.
1. **Avatar emblem micro** sits quietly in the top right corner of the entry. Tapping it does nothing in v1 — it simply shows which avatar’s world the user is in. Future: could allow avatar switch from within the Journal.
1. **Save** — single button. *“Keep this”* — Montserrat, avatar accent colour. No secondary cancel button. Swipe down to discard without saving (standard iOS / Android gesture).
